...
def predict_img(net, full_img, device, scale_factor=1, out_threshold=0.5):
    ...
    with torch.no_grad():
        output = net(img).cpu()  # CPU for now, we'll handle MPS in the device setting
...
if __name__ == "__main__":
    ...
    # Set up the model and device for inference
    net = UNet(n_channels=3, n_classes=args.classes, bilinear=args.bilinear)

    # Change the device to MPS for MacBook Pro M3 if available
