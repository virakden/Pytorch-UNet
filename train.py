...
def train_model(
    ...
    loader_args = dict(
        batch_size=batch_size, num_workers=os.cpu_count(), pin_memory=False
    )  # Pin memory set to False for CPU
    ...
    grad_scaler = None  # AMP not supported on MacBook M3 (No CUDA)
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f"Epoch {epoch}/{epochs}", unit="img") as pbar:
            for batch in train_loader:
                ...
                # No AMP (mixed precision) support on M3, so just regular forward pass
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        for tag, value in model.named_parameters():
                            ...
                            # Check if the gradient exists (is not None)
                            if value.grad is not None:
                                ...
                                print(f"Warning: No gradient found for {tag}")

                            # Check for weights without NaN or Inf values
                        ...
                        val_score = evaluate(
                            model, val_loader, device, amp=False
                        )  # AMP is disabled
        ...
        if save_checkpoint:
            ...
            state_dict["mask_values"] = dataset.dataset.mask_values
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.error(
            ...
            "Consider enabling AMP (--amp) for faster and memory-efficient training"
        )
